...
const ensureRepoInSyncWithOrigin = () => {
  const taskEnd = log.taskStart('Ensuring your local repo is in sync with origin');
...
const commitRelease = (version) => {
  const taskEnd = log.taskStart('Commiting and pushing release to origin');
...
const tagRelease = (version, changes) => {
  const taskEnd = log.taskStart('Tagging the release');
